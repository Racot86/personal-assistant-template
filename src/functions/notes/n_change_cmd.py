...
def n_change_cmd(cmd):
    ...
    if not notes.note_exists(title): 
        a_print("Note with provided title not found", prefix="TARDIS: ", main_color=Settings.error_color)
    else:
        print("TARDIS: ")
        ...
        a_print("Title updated successfully!", prefix="TARDIS: ", main_color=Settings.success_color)
        ...
        print(f"{Settings.shadow_color}Body:{Settings.end_all}\n{Settings.notes_color}\n{new_text}\n{Settings.end_color}\n")
        a_print("Note updated successfully!!", prefix="TARDIS: ", main_color=Settings.success_color)
        storage.save_note_book(notes)
